...
int LinkedList_insert(LinkedList *list, const DataType item) {
	...
	if (list->tam_atual == 0) { 
		...
		if (list->cursor == list->top) {
			...
			node->prevPtr         = list->cursor->prevPtr;
			list->cursor->prevPtr = node;
...
int LinkedList_remove(LinkedList *const list) {
	...
	if (!LinkedList_isEmpty(list)) {
		if (list->tam_atual == 1) {
			...
			if (list->cursor == list->top) {
				list->top    = list->cursor->nextPtr;
				list->cursor = list->cursor->nextPtr;
				free(list->cursor->prevPtr);
				list->cursor->prevPtr = NULL;
				printf("cc");
			} else {
				LinkedList *aux                = (struct LinkedList *)list->cursor->nextPtr;
				list->cursor->nextPtr->prevPtr = (struct ListNode *)list->cursor->prevPtr;
				list->cursor->prevPtr->nextPtr = list->cursor->nextPtr;
				free(list->cursor);
...
int LinkedList_gotoEnd(LinkedList *const list) {
	assert(list);
	if (!LinkedList_isEmpty(list)) {
		while (list->cursor->nextPtr != NULL) {
...
bool LinkedList_gotoPrior(LinkedList *list) {
	assert(list);
	if (list->cursor->prevPtr != NULL) {
		list->cursor = list->cursor->prevPtr;
		return 1;
	} else {
		return 0;
	}
